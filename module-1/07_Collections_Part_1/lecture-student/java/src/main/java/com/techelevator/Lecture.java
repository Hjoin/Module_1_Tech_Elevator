...
public class Lecture {
	...
	public List<Double> removeLastNumber(List<Double> numbers) {
		List<Double> result = new ArrayList<>(numbers); //copies elements from numbers into result
		result.remove(numbers.size() - 1);
		return result;
	...
	public int countTrues(List<Boolean> answers) {
		int occurrences = Collections.frequency(answers, true);
		return occurrences;
	}
