...
public class Exercises {
	...
	public boolean firstLast6(int[] nums) {
		boolean firstOrLastElement = false;
		for (int i = 0; i < nums.length; i++){
			if (nums[0]==6 || nums[nums.length-1]==6){
				firstOrLastElement = true;
			}
		}
	...
	public int[] maxEnd3(int[] nums) {
		int maxNumber = Math.max(nums[0], nums[2]);
		return new int[] {maxNumber, maxNumber, maxNumber};
	}
	...
	public int sum13(int[] nums) {
		int sumWithout13AndWhatsAfter = 0;
		for (int i = 0; i < nums.length; i++){
			if (nums.length > 0){
				if(nums[i] != 13){
					sumWithout13AndWhatsAfter += nums[i];
				} else{
					i++;
				}
			}
		}
		return sumWithout13AndWhatsAfter;
	}
	...
	public boolean has22(int[] nums) {
		boolean twoNextToATwo = false;
		for (int i = 0; i < nums.length - 1; i++){
			if (nums[i] == 2 && nums[i + 1] == 2){
				twoNextToATwo = true;
			}
		}
		return twoNextToATwo;
	}
	...
	public boolean lucky13(int[] nums) {
		boolean sumWithoutOneOrThree = true;
		for (int i = 0; i < nums.length; i++){
			if ((nums[i] == 1) || (nums[i] == 3)){
				sumWithoutOneOrThree = false;
			}
		}
		return sumWithoutOneOrThree;
	}
	...
	public boolean sum28(int[] nums) {
		...
		for (int i = 0; i < nums.length; i++) {
			if (nums[i] == 2) {
				addingTheTwos += nums[i];
			}
		}
			return (addingTheTwos == 8);
		}
